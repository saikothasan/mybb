            ...
            case 'pgsql':
                $db->write_query(
                    "CREATE TABLE " . TABLE_PREFIX . "mybbmessenger_relation(
                        ...
                        lms timestamp NOT NULL default CURRENT_TIMESTAMP,
                        PRIMARY KEY (id)
                    );"
                );
                break;
...
function mybbmessenger_activate()
{
	...
	//Allowed User Group
	$mybbmessnger_allowed_group = array(
		'sid' => 'NULL',
		'name' => 'mybbmessnger_allowed_group',
	...
	$db->insert_query('settings',$mybbmessnger_allowed_group);
	rebuild_settings();
...
function mybbmessenger_start()
{
	...
	$allowed_group = explode(',', $mybb->settings['mybbmessnger_allowed_group']);	
	if($usergroup!=1 && $settings['mybbmessenger_enable']==1 && (in_array($usergroup, $allowed_group) || in_array('-1', $allowed_group))) {
